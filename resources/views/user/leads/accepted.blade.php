    ...
    <div class="card">
      ...
      <div class="table-responsive py-4">
        <table class="table table-flush" id="datatable-basic">
          ...
          <tbody>
            @foreach (App\Models\Lead::where('user_id',Auth::user()->id)->where('plot_id','!=',null)->where('accepted',true)->get() as $lead)
            <tr>
              <td>{{$lead->name}}</td>
              <td>{{$lead->email}}</td>
              <td>{{$lead->message}}</td>
      ...
      <div class="card">
        ...
        <div class="table-responsive py-4">
          <table class="table table-flush" id="datatable-basic">
            ...
            <tbody>
                @foreach (App\Models\Lead::where('user_id',Auth::user()->id)->where('property_id','!=',null)->where('accepted',true)->get() as $lead)
              <tr>
                <td>{{$lead->name}}</td>
                <td>{{$lead->email}}</td>
                <td>{{$lead->message}}</td>
